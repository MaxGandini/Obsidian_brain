...
for csv_file_path in tables:
    # Ensure the file exists before attempting to load it
    if csv_file_path.exists():
        try:
            # Dynamically insert the file path into the COPY SQL statement
            copy_sql = f"""
            ...
            conn.commit()
            print(f"Data from {csv_file_path} successfully loaded into the table.")
